...
class LineGraphSage(nn.Module):
    ...
    def __init__(self,
        ...
        super(LineGraphSage, self).__init__()
        ...
        for mp in range(self.n_mp):
            ...
            for i in range(len(node_layer_specs)):
                ...
                node_agg = nn.ModuleList([aggregator_class(
                    input_dim=input_dim,
                    edge_dim=edge_spec['output_dim'] * n_head,
    ...
    def forward(self, train_ids, train=True):
        ...
        for mp in range(self.n_mp):
            ...
            all_feats = self.prep(all_ids, all_feats, layer_idx=0)
            ...
            #prerequisite for updating node
            node_requires1=[train_ids]
            edge_requires1=[node2edge_idx[train_ids].contiguous().view(-1)]

            #prerequisite for updating edge
            node_requires2=[edge2node_idx[edge_requires1[-1]]]
            edge_requires2=[edge_neigh[edge_requires1[-1]]]

            #layer-wise update requirement
            for layer_idx in range(self.depth-1):
                node_req = torch.cat([node_requires1[-1],node_requires2[-1]])
                edge_req = torch.cat([edge_requires1[-1],edge_requires2[-1]])

                node_requires1.append( node_req )
            ...
            for layer_idx in range(self.depth):
                ...
                for chunk_id, trunk in enumerate(torch.split(edge_ids, self.train_batch, dim=0)):
                    chunk_feat = all_edges[trunk]
                    neigh_feat = all_edges[edge_neigh[trunk].contiguous().view(-1)]
                    chunk_node = all_feats[edge2node_idx[trunk].contiguous().view(-1)]
                ...
                tmp_feats = []
                for chunk_id, trunk in enumerate(torch.split(all_ids, self.train_batch, dim=0)):
                    chunk_feat = all_feats[trunk]
                    neigh_feat = all_feats[node_neigh[trunk].contiguous().view(-1)]
                    chunk_edge = all_edges[node2edge_idx[trunk].contiguous().view(-1)]
