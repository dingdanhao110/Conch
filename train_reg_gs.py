...
from model.models import conch_dgi_gs
import math
# --
# Helpers
...
def evaluate(model, problem, batch_size, loss_fn, coff, mode='val'):
    ...
    for (ids, targets, _) in problem.iterate(mode=mode, shuffle=False, batch_size=batch_size):
        ...
        pred = model(ids, problem.feats, feat2=None, msk=None, samp_bias1=None, samp_bias2=None, get_embed='pred')
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        for ids, targets, epoch_progress in problem.iterate(mode='train', shuffle=True, batch_size=args.batch_size):
            ...
            optimizer.zero_grad()
            preds,weights,reg = model(ids, X, X_tilda, None, None, None, get_embed=None)
            ...
            #lbl for[ids]
            lbl_1 = torch.ones(n_mp, ids.shape[0])
            lbl_2 = torch.zeros(n_mp, ids.shape[0])
            lbl = torch.cat((lbl_1, lbl_2), 1)
            if torch.cuda.is_available() and args.cuda:
                lbl = lbl.cuda()

            loss = problem.loss_fn(preds, targets.squeeze())+bce(reg,lbl)*coff
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(model.parameters(), 5)
            optimizer.step()
