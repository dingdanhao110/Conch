...
def evaluate(model, problem, batch_size, loss_fn,coff, mode='val'):
    ...
    for (ids, targets, _) in problem.iterate(mode=mode, shuffle=False, batch_size=batch_size):
        ...
        pred, _, reg_loss = model(ids, train=False)
        loss += loss_fn(pred, targets.squeeze()).item()+reg_loss.item()*coff
...
def parse_args():
    ...
    parser.add_argument('--prep-class', type=str, default='node_embedding')  # linear,identity,node_embedding
    ...
    parser.add_argument('--max-coff', type=float, default=5e-7)
    parser.add_argument('--max-epoch', type=float, default=100)
    parser.add_argument('--coff-exp', type=float, default=10)
